  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="p-4 bg-obsidian-900 border border-obsidian-700 rounded-xl">
          ...
          <div className="space-y-3">
            {summary.thrivingHabits.length > 0 ? (
              summary.thrivingHabits.map((item) => (
                <div
                  key={item.habitId}
                  className="p-3 bg-obsidian-800 border border-obsidian-700 rounded-xl flex items-center justify-between"
                >
                  <div>
            ...
            {summary.strugglingHabits.length > 0 ? (
              summary.strugglingHabits.map((item) => (
                ...
                >
                  ...
                  <div className="text-xs font-mono font-bold text-zinc-400 bg-obsidian-900 px-2 py-1 rounded border border-obsidian-700">
        ...
        <div className="grid grid-cols-7 gap-2">
          {summary.dayOfWeekRates.map((d) => (
            <div key={d.day} className="flex flex-col items-center gap-2">
              <div className="w-full bg-obsidian-800 rounded-lg h-24 relative flex items-end p-1 border border-obsidian-700">
                <div
                  className="w-full bg-gradient-to-t from-flame-600 to-flame-400 rounded transition-all"
                  style={{ height: `${d.rate}%` }}
                />
              </div>
